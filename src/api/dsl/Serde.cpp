...
std::shared_ptr<Expression> u_expr(const std::string& alias,
  ...
  switch (ut) {
  case UDFType::LIKE: {
    return as(alias, std::make_shared<LikeExpression>(inner, custom, flag, flag2));
  }
  ...
  case UDFType::ROUNDTIME: {
    ...
    int64_t unit = stoll(custom, 0, comma_pos);
    N_ENSURE_GT(custom.length() - comma_pos - 1, 0, "beginTime is an empty string");
    int64_t begin = stoll(custom, comma_pos + 1, custom.length() - comma_pos - 1);
