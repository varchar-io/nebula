...
LoadResult LoadHandler::loadConfigured(const LoadRequest* req, LoadError& err, std::string& name) {
  ...
  while (p.size() > 0) {
    ...
    std::vector<SpecSplitPtr> splits{ std::make_shared<SpecSplit>(path, 0, watermark) };
...
LoadResult LoadHandler::loadGoogleSheet(const LoadRequest* req, LoadError& err, std::string& name) {
  ...
  std::vector<SpecSplitPtr> splits{ std::make_shared<SpecSplit>(sheet.url, sheet.rows, 0) };
  auto spec = std::make_shared<IngestSpec>(tbSpec, "0", sheet.uid, splits, SpecState::NEW);
...
LoadResult LoadHandler::loadDemand(const LoadRequest* req, LoadError& err, std::string& name) {
  ...
  std::vector<SpecSplitPtr> splits{ std::make_shared<SpecSplit>(demand.path, 0, 0) };
  auto spec = std::make_shared<IngestSpec>(tbSpec, "0", demand.domain, splits, SpecState::NEW);
