...
// compute bits needed for given percentile values in data
uint32_t RleEncoder::percentileBits(int64_t* data, size_t offset, size_t length, double p, bool reuseHist) {
  if ((p > 1.0) || (p <= 0.0)) {
    throw NException(fmt::format("Invalid percentile: {0}", p));
  }
  ...
  if (!reuseHist) {
    ...
    // compute the histogram based on voting
    for (size_t i = offset, end = offset + length; i < end; i++) {
...
void RleEncoder::initializeLiterals(int64_t val) {
  literals_[numLiterals_++] = val;
  fixedRunLength_ = 1;
  variableRunLength_ = 1;
}

void RleEncoder::computeZigZagLiterals(EncodingOption& option) {
  int64_t zzEncVal = 0;
  for (size_t i = 0; i < numLiterals_; i++) {
    if (isSigned_) {
      zzEncVal = zigZag(literals_[i]);
    } else {
      zzEncVal = literals_[i];
    }
    zigzagLiterals_[option.zigzagLiteralsCount++] = zzEncVal;
  }
}

void RleEncoder::determineEncoding(EncodingOption& option) {
  ...
  if (numLiterals_ <= MIN_REPEAT) {
    ...
    option.zzBits100p = percentileBits(zigzagLiterals_, 0, numLiterals_, 1.0);
  ...
  for (size_t i = 1; i < numLiterals_; i++) {
    ...
    auto absDelta = std::abs(currDelta);
    if (i > 1) {
      adjDeltas_[option.adjDeltasCount++] = absDelta;
      deltaMax = std::max(deltaMax, absDelta);
    }
  }
  ...
  if (option.min == max) {
    if (!option.isFixedDelta) {
      throw NException(fmt::format("{0}=={1}, isFixedDelta cannot be false", option.min, max));
    }

    if (currDelta != 0) {
      throw NException(fmt::format("{0}=={1}, currDelta should be zero", option.min, max));
    }
    option.fixedDelta = 0;
  ...
  if (option.isFixedDelta) {
    if (currDelta != initialDelta) {
      throw NException("currDelta should be equal to initialDelta for fixed delta encoding");
    }
  ...
  computeZigZagLiterals(option);
  option.zzBits100p = percentileBits(zigzagLiterals_, 0, numLiterals_, 1.0);
  option.zzBits90p = percentileBits(zigzagLiterals_, 0, numLiterals_, 0.9, true);
  ...
  if (diffBitsLH > 1) {
    ...
    for (size_t i = 0; i < numLiterals_; i++) {
      baseRedLiterals_[option.baseRedLiteralsCount++] = (literals_[i] - option.min);
    }

    // 95th percentile width is used to determine max allowed value
    // after which patching will be done
    option.brBits95p = percentileBits(baseRedLiterals_, 0, numLiterals_, 0.95);

    // 100th percentile is used to compute the max patch width
    option.brBits100p = percentileBits(baseRedLiterals_, 0, numLiterals_, 1.0, true);
...
void RleEncoder::writeByte(char c) {
  buffer_.write(bufferPos_++, c);
}

void RleEncoder::writeShortRepeatValues(EncodingOption&) {
  int64_t repeatVal;
  if (isSigned_) {
    repeatVal = zigZag(literals_[0]);
  } else {
  ...
  for (int32_t i = static_cast<int32_t>(numBytesRepeatVal - 1); i >= 0; i--) {
    writeByte(static_cast<char>((repeatVal >> (i * 8)) & 0xff));
  }

  fixedRunLength_ = 0;
}

void RleEncoder::writeInts(int64_t* input, uint32_t offset, size_t len, uint32_t bitSize) {
...
void RleEncoder::writeDirectValues(EncodingOption& option) {
  ...
  writeInts(zigzagLiterals_, 0, numLiterals_, fb);

  // reset run length
  variableRunLength_ = 0;
}

void RleEncoder::writePatchedBasedValues(EncodingOption& option) {
  ...
  writeInts(baseRedLiterals_, 0, numLiterals_, closestFixedBits);
...
void RleEncoder::writeDeltaValues(EncodingOption& option) {
  uint32_t len = 0;
  ...
  if (isSigned_) {
    writeVslong(literals_[0]);
  } else {
    writeVulong(literals_[0]);
  }
  ...
  if (option.isFixedDelta) {
    ...
    writeInts(adjDeltas_, 1, numLiterals_ - 2, fb);
  }
}

void RleEncoder::writeValues(EncodingOption& option) {
  if (numLiterals_ != 0) {
...
void RleEncoder::preparePatchedBlob(EncodingOption& option) {
...
void RleEncoder::write(int64_t val) {
  ...
  const auto currentDelta = val - literals_[numLiterals_ - 1];
  ...
  if (fixedRunLength_ > 0 && fixedRunLength_ < MIN_REPEAT && val != literals_[numLiterals_ - 1]) {
  ...
  if (numLiterals_ != 0) {
    ...
    if (variableRunLength_ != 0) {
      ...
      if (fixedRunLength_ < MIN_REPEAT) {
        ...
        determineEncoding(option);
        writeValues(option);
      } else if (fixedRunLength_ >= MIN_REPEAT
